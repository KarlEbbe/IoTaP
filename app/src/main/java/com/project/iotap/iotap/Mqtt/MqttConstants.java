package com.project.iotap.iotap.Mqtt;

/**
 * MqttConstants used in the app shared by different classes.
 */
public class MqttConstants {

    public static final String MQTT_BROKER_URL = "tcp://m23.cloudmqtt.com:15915";
    public static final String CLIENT_ID = "android";
    public static final String CLIENT_USER = "hlmtgqdu";
    public static final String CLIENT_PASSWORD = "ZbyyNuAPignO";
    public static final String GREETING_TOPIC = "Greeting";
    public static final String GREETING = "1:";
    public static final String COMMAND = "2:";
    public static final String DISCONNECT = "3:";
    public static final int QOS = 1;
}